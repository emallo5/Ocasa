...
public class ColumnDAO extends GenericDAOImpl<Column, String> {
    ...
    public List<Column> findLogicColumnsForLayout(String layoutId){

        try {
            QueryBuilder<Column, String> columnQuery = dao.queryBuilder();
            columnQuery.where().eq("logic", true);

            QueryBuilder<LayoutColumn, Long> layoutQuery = new LayoutColumnDAO(context).getDao().queryBuilder();
            layoutQuery.where().eq("layout_id", layoutId);

            columnQuery.join(layoutQuery);
